...
class Controller:
    ...
    async def link(self, msg, item=None, *args):
        ...
        """
        if not channel.guild.me.permissions_in(msg.channel).embed_links:
            await msg.channel.send(**{
    ...
    async def snapshot(self, msg, *args):
        ...
        """
        if not channel.guild.me.permissions_in(msg.channel).attach_files:
            await msg.channel.send(**{
    ...
    async def location(self, msg, place_name=None, *args):
        ...
        if place_name.lower() in MapController.locations:
            ...
            if channel.guild.me.permissions_in(msg.channel).embed_links:
            ...
            if channel.guild.me.permissions_in(msg.channel).attach_files:
...
async def on_message(message):
    ...
    if intent == Intent.DIRECT:
        ...
        if not channel.guild.permissions_in(message.channel).attach_files:
            await msg.channel.send(**{
                'content': 'Cannot send images on this channel',
                'reference': msg.to_reference(),
                'mention_author': True,
